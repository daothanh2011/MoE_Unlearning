...
# across all 3 ColoredMNIST leave-one-out experiments.
#
# Usage:
...
CONFIG="${CONFIG:-configs/phase2_cmnist_config.yaml}"
CONDA_ENV="${CONDA_ENV:-gmoe}"

# Which environments to sweep (space-separated, default all 3)
ENVS="${ENVS:-0 1 2}"
...
# ── Logging ──────────────────────────────────────────────────
LOG_DIR="logs/cmnist_ablation_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
echo "Logs: $LOG_DIR"
...
echo "============================================================"
echo "CMNIST ablation sweep complete. Results in $LOG_DIR"
echo "============================================================"
